...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Ejecta Yield}

The integral yield as a function of ejecta particle mass $Y_{\mathcal{C}}$ is computed by dividing the cumulative amount of mass $M_{\mathcal{C}}$ of fragments of mass greater than $m_{ej}$ by the impactor mass $m_{imp}$
\begin{align}\label{eq:YC(m)def}
Y_{\mathcal{C}}(>m_{ej}) &= \frac{1}{m_{imp}}M_{\mathcal{C}}\\\nonumber
&= \frac{1}{m_{imp}}\int_{v_{min}}^{v_{max(m_{ej})}}dv \frac{dM}{dv}f(m_{ej},v),
\end{align}
where $dM/dv$ is the amount of mass ejected at ejecta speed $v$ (Equation \eqref{eq:dM/dv final}) and $f(m_{ej},v)$ is the distribution of fragments ejected at speed $v$ (Equation \eqref{eq:f(m,mbv)}). The upper limit is (specifying $m_{ej}$ as the ejecta mass as opposed to the impactor mass $m_{imp}$)
\begin{equation}
v_{max}(m_{ej}) = \min\left[v_{min}(m_{ej}/m_b)^{-1/\delta}, v_{max}\right].
\end{equation}
Expanding Equation \eqref{eq:YC(m)def} and changing variables to a normalized crater position $y = x / (n_2 R)$,
\begin{align}\label{eq:YC(m)final}
&Y_{\mathcal{C}}(>m_{ej}) = \frac{3k}{4\pi}\frac{\rho}{\delta}\left[\left(\frac{x(v_{min})}{a}\right)^3 - \left(\frac{x(v_{max}(m_ej))}{a}\right)^3\right]\\\nonumber
&- A_{\mathcal{C}}
\left(\frac{m_{ej}}{m_b}\right)^{\frac{\beta}{3}}
\left[\beta\left(\frac{x(v_{min})}{n_2 R}; -\frac{\beta\delta}{3\mu}+3, \frac{\beta\delta p}{3}+1\right)
- \beta\left(\frac{x(v_{max}(m_{ej}))}{n_2 R}; -\frac{\beta\delta}{3\mu}+3, \frac{\beta\delta p}{3}+1\right)\right],
\end{align}
where the constants with respect to $m_{ej}$ are
\begin{equation}
A_\mathcal{C} = \frac{9k}{4\pi}\left(\frac{\rho}{\delta}\right)^{-\frac{\beta\delta\nu}{3\mu}+1}
\left(\frac{C_1 U}{v_{min}}\right)^{\frac{\beta\delta}{3}}
\left(\frac{a}{n_2 R}\right)^{\frac{\beta\delta}{3\mu}-3}.
\end{equation}
The ejecta yield for ejecta fragments greater than $m_{ej}$, $Y_{\mathcal{C}}(>m_{ej})$, approaches the total ejecta yield $M_{tot}/{m_{imp}}$ (see Equation \eqref{eq:Mtot}) as $m_{ej}\to 0$ and letting $v_{min}\to 0$, which is to be expected.


The example shown in Figure \ref{fig:IntegralEjectedMass_vs_ParticleMass} is done numerically, taking advantage of the identity
\begin{equation}
...
\begin{figure}[!htb]
	...
	\caption{The ejecta yield for particle mass $>\bar{m}_{ej}$ (Equation \eqref{eq:YC(m)final}) vs.\ ejecta particle mass $\bar{m}_{ej} = m_{ej}/m_b$ for a lunar regolith target (using SFA in Table~\ref{tab:scaling law parameters}) with regolith surface bulk density $\rho = 1.3$ g/cc, impactor density of $4.0$ g/cc, and impactor speed of $20$ km/s.}\label{fig:IntegralEjectedMass_vs_ParticleMass}
\end{figure}


The impactor mass distribution can also be included with the ejecta yield to give a better picture of how the realistic yield might look like, see Figure \ref{fig:EjectaYieldImpactorMassCDF_vs_ImpactorSpeed_vs_Mass_PS1e-6_4gcc}. Typical sporadic meteoroid speeds range from just under $10$ km/s to about $70$ km/s for impactor mass sizes of $10^{-6}$ g to $10$ g. 
...
\begin{figure}	
	...
	\begin{subfigure}[t]{.48\textwidth}
		\centering
		% include third image
		\includegraphics[width=.98\linewidth]{EjectaYield_vs_ImpactorSpeed_vs_Mass_SFA1e-3_4gcc.png}  
		%\caption{Put your sub-caption here}
	...
	\begin{subfigure}[t]{.48\textwidth}
		\centering
		% include fourth image
		\includegraphics[width=.98\linewidth]{EjectaYield_vs_ImpactorSpeed_vs_Mass_WCB1e-3_4gcc.png}  
		%\caption{Put your sub-caption here}
...
\begin{subfigure}[t]{.48\textwidth}
	\centering
	% include third image
	\includegraphics[width=.98\linewidth]{EjectaYield_vs_ImpactorSpeed_vs_Mass_SFA1e-0_4gcc.png}  
	%\caption{Put your sub-caption here}
	\label{fig:EjectaYield_vs_ImpactorSpeed_vs_Mass_7}
\end{subfigure}
\begin{subfigure}[t]{.48\textwidth}
	\centering
	% include fourth image
	\includegraphics[width=.98\linewidth]{EjectaYield_vs_ImpactorSpeed_vs_Mass_WCB1e-0_4gcc.png}  
	%\caption{Put your sub-caption here}
	\label{fig:EjectaYield_vs_ImpactorSpeed_vs_Mass_8}
\end{subfigure}

	\caption{The ejecta yield is shown for SFA (first column) and WCB (second column) as a function of impactor speed $U$ and impactor mass $m_{imp}$ for ejecta fragments masses greater than $1\times 10^{-6}$~g (row 1), $1\times 10^{-3}$~g (row 2), and $1\times 10^{0}$~g (row 3).}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=1.\linewidth]{EjectaYieldImpactorMassCDF_vs_ImpactorSpeed_vs_Mass_PS1e-6_4gcc.png}
	\caption{The ejecta yield convoluted with the impactor mass distribution (i.e., the Gr{\"u}n interplanetary flux) is shown as a function of impactor speed $U$ and impactor mass $m_{imp}$ for ejecta fragments greater than $10^{-6}$ g. The impactor mass distribution is normalized by the interplanetary flux at $10^{-6}$ g.}\label{fig:EjectaYieldImpactorMassCDF_vs_ImpactorSpeed_vs_Mass_PS1e-6_4gcc}
\end{figure}
