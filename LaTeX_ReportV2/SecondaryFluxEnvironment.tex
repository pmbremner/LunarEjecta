...
\begin{itemize}
	\item If $r_s(D) > h_{st}$, then $sign(F) = -1$ and $r_s = h_{st}$ (hitting the top),
	\item Else if $r_s(D) < h_{sb}$, then $sign(F) = +1$ and $r_s = h_{sb}$ (hitting the bottom),
	\item Else
	\begin{itemize}
		\item If $\gamma_p > \gamma_{p,ap}(r_s(D))$ (Equation \eqref{eq:gamma p ap 1}), then $sign(F) = -1$ and $r_s = r_s(D)$ (hitting the side above the local horizon),
		\item Else $sign(F) = +1$ and $r_s = r_s(D)$ (hitting the side below the local horizon).
	\end{itemize}
\end{itemize}
