...
f_{\mathcal{C}}(m_{ej}) &= \int^{x(v_{min})}_{x(v_{max}(m))}
...
\end{equation}
Because of the complicated bounds in Equation \ref{fig:IntegralEjectedMass_vs_ParticleMass}, a closed form solution of $Y_{\mathcal{C}}(>m_{ej,i})$ is not possible, in general. The example shown in Figure \ref{fig:IntegralEjectedMass_vs_ParticleMass} is done numerically, taking advantage of the identity
\begin{equation}
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=1.\linewidth]{IntegralEjectedMass_vs_ParticleMass.png}
