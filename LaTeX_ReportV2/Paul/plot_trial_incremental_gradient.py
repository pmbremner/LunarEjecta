...
td_x4 = np.linspace(0., 300., num=3*numvals, endpoint=True, dtype=np.float)
td_y4 = np.ones(3*numvals, dtype=np.float)
...
for i,x in enumerate(td_x4):
    if x >=0 and x <= 100.:
        td_x1.append(x)
        td_y1.append(td_y4[i])
    elif x > 100. and x <= 200.:
        td_x2.append(x)
        td_y2.append(td_y4[i])
    elif x > 200. and x <= 300.:
        td_x3.append(x)
        td_y3.append(td_y4[i])
...
mm_ax_xticks = mm_ax.get_xticks() #(minor=False)e
