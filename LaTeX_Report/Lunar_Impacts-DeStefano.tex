...
\end{equation}
and we notice that this is the integral for the incomplete beta function. Therefore, the contribution of secondary ejecta as a function of distance is given by
\begin{align}
...
The integral over the secondary ejecta as a function of speed is given by
\begin{equation}
\Phi(D)\int_{v_{min}}^{v_{max}}\int_{\gamma_{min}}^{\gamma_{max}}d\gamma\sin\gamma dv H(v).
\end{equation}
We must break up the integral over the zenith angle into two regions, one less than the optimal angle $\gamma_{opt}$ and one greater than. In the limit as the ROI size goes to zero, the region between the two cases goes away, so we will not worry about it here.
