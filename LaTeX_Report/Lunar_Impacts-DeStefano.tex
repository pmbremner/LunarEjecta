...
\subsubsection{Ejecta Mass Distribution Function}
...
H(v) = 3\mu v^{-(3\mu+1)}.
\end{equation}
For the angular dependent terms $F(\alpha)G(\beta)$, they technically should depend on speed as well as particle size. For simplicity, we will assume the angular distribution is independent of speed and particle size but will be dependent on impact zenith and azimuth angle with respect to the bearing.
...
\begin{figure}[h!]
	...
	\caption{blah}\label{fig:Distance_vs_EjectaSpeed_and_ZenithAngle}
\end{figure}
...
D = 4\gamma r_m.
\end{equation}
For example, from this equation we can conclude that for $\gamma < 45^\circ$, the ejecta will not reach the antipodal point.
