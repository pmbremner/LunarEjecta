...
\end{equation}
and solving for $\beta-\beta_i$
\begin{equation}
\arccos\left(\frac{\alpha_{max,0}+\alpha_{max,\pi}}{\alpha_{max,\pi}-\alpha_{max,0}}\right) = \beta-\beta_i.
\end{equation}
As a simplification, we can approximate the $\cos(\beta-\beta_i)$ term as
\begin{equation}
\cos(\beta-\beta_i) \sim 1 - \frac{\beta-\beta_i}{\pi/2},
\end{equation}
for $0 \le \beta-\beta_i \le \pi$ such that $\cos\alpha_{max}$ becomes
\begin{equation}
