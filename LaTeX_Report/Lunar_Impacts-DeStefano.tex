...
\subsubsection{A relook at the Particle Size Distribution Function}
...
Given two latitude-longitude points on a sphere, $(\phi_1, \lambda_1)$ and $(\phi_2, \lambda_2)$, we can compute the distance and bearing following Chris Veness's webpage\footnote{\url{https://www.movable-type.co.uk/scripts/latlong.html}}.
...
\end{equation}
for $\Delta\phi = \phi_1-\phi_2$ and $\Delta\lambda = \lambda_1-\lambda_2$. Solving for the distance and simplifying, we have
\begin{equation}
...
\lambda_2 &= \lambda_1 + \arctan\left[\frac{\sin\theta\sin(D/r_m)\cos\phi_1}{\cos(D/r_m) - \sin\phi_1\sin\phi_2}\right], \text{ or}\\
&= \lambda_1 + \arctan\left[\frac{\sin\theta\sin(D/r_m)}{\cos(D/r_m)\cos\phi_1 - \sin(D/r_m)\sin\phi_1\cos\theta}\right].
\end{align}
...
\tan\beta_s = \frac{2\frac{v_p^2}{v_{esc}^2}\frac{r_m}{r_s}\sin\gamma_p\cos\gamma_p}{2\frac{v_p^2}{v_{esc}^2}\frac{r_m}{r_s}\sin^2\gamma_p-1}F_s(r_s,v_p,\gamma_p),
\end{equation}
where the factor $F_s$ is given by
\begin{equation}
F_s(r_s,v_p,\gamma_p) = \sqrt{1 + 
	\frac{\frac{r_s}{r_m}-1}{\cos^2\gamma_p}\left[1+\frac{r_s}{r_m}\left(1-\frac{v_{esc}^2}{v_p^2}\right)\right]}.
\end{equation}
%\begin{equation}
...
e\sin\beta_s &= \sqrt{e^2 - (e\cos\beta_s)^2}\\
 &= 2\frac{v_p^2}{v_{esc}^2}\frac{r_m}{r_s}\sin\gamma_p\cos\gamma_p
F_s(r_s,v_p,\gamma_p),
\end{align}
such that
\begin{equation}
\cot\gamma_s = \cot\gamma_p F_s(r_s,v_p,\gamma_p).
\end{equation}
...
\begin{equation}\label{eq:rmax_satellite}
...
To reach a particular satellite radius $r_s=r_{\text{max}}$ for a given ejecta angle $\gamma_p$, we have the following condition on the ejecta speed $v_p$ (solving for $v_p$ in Equation \eqref{eq:rmax_satellite}):
%\begin{equation}
