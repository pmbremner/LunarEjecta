...
N_{ej}(x) \sim \int_{x}^{\infty}\frac{dx}{m(x)}\frac{d(P_{\text{Moon}}(x))}{dx},
\end{equation}
...
\begin{equation}
P_{\text{Moon}}(x) = y_i\left(\frac{x}{x_i}\right)^{b_i},
\end{equation}
where
\begin{equation}
b_i = \frac{\log(y_{i+1}/y_i)}{\log(x_{i+1}/x_i)},
\end{equation}
and where $x_i \le x \le x_{i+1}$, for data $x_i$ from Figure \ref{fig:Carrier2003_Fig1_particle-size-distribution}. Therefore, we can write Equation~\eqref{eq:N_ej_carrier1} as
