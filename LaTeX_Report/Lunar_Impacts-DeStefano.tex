...
The azimuth distribution is given by \citep{rival1999modeling}
\begin{equation}
G(\beta) =
\begin{cases}
...
\end{equation}
such that $F'(\alpha_{max}) = 0$ and $F''(\alpha_{max}) < 0$. We then can do a variable substitution (chosen so the domain goes from $\alpha\in [0,\pi/2]$ to $x\in [0,1]$)
\begin{align}
1-x &= \cos\alpha,\\
